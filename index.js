...
//testing
// const key = 're_1959tCvu_JK5CNMQ3xthTitvWVRbLpQ27'
...
const sendEmail = (emailAddress, emailData) => {
  const transporter = nodemailer.createTransport({
    service: 'gmail',  //changed
    host: "smtp.gmail.com", //changed
    port: 587,
    secure: false, // true for port 465, false for other ports
    auth: {
      user: process.env.TRANSPORTER_EMAIL, //changed
      pass: process.env.TRANSPORTER_PASS, //changed
    },
  });
  ...
  const mailBody = {
    from: `"StayVista" <${process.env.TRANSPORTER_EMAIL}>`, // sender address  //changed
    to: emailAddress, // list of receivers //changed
...
async function run() {
  try {
    ...
    const verifyAdmin = async (req, res, next) => {
      ...
      if (!result || result?.role !== 'admin') return res.status(401).send({ message: 'unauthorized access mama!!' })
      next()
    }
    ...
    const verifyHost = async (req, res, next) => {
      ...
      if (!result || result?.role !== 'host') return res.status(401).send({ message: 'unauthorized access mama!!' })
      next()
    }
    ...
    app.get('/rooms', async (req, res) => {
      ...
      // console.log('from category', typeof category); // string
      let query = {}
      // console.log('from query', query);
      if (category && category !== 'null') query = { category } // niche ta ar ata same e
      // if (category && category !== 'null') {  // aikhane check korar karon  null string akare astase
      //   query = { category: category }
      //   console.log('from query fn', query);
      // }
    ...
    app.get('/my-listings/:email', verifyToken, verifyHost, async (req, res) => {
      const email = req.params.email;
      let query = { 'host.email': email } // akhane queatation er bitorei 'host.email' likhte hobe
    ...
    // Get a single room data form db using _id
    ...
    app.get('/admin-stat', verifyToken, verifyAdmin, async (req, res) => {
      ...
      chartData.unshift(['Day', 'Sales']) // duitai same output dibe
...
//   //   to: 'emonownmail17@gmail.com', // taka pay na korle shudu ai email ai pathano jabe
//   //   subject: 'Hello World',
...
//     // to: ['emonownmail17@gmail.com'],
//     to: ['hayopi5295@maonyn.com'],
//     subject: 'Hello World',
